...
class circuit =
  object(self)
    ...
    method get_func f =
      match IM.find_opt f funcs with
      | Some fn -> fn
      | None ->
	  let fn = next_func in
    ...
    method get_rel r =
	  ...
	  s*rn
    method get_var x =
      match IM.find_opt x vars with
      | Some node -> node
      | None ->
	  let node =
	    match Cnf.Im.find_opt x sub with
	    | None ->
		let n = next_node in
    ...
    method get_term = function
	  ...
	  match LIM.find_opt (fn, subnodes) terms with
	  | Some node -> node
	  | None ->
    ...
    method get_lit (r, subterms) =
      ...
      print_string "\n";
      Format.print_flush (); *)
      match LIM.find_opt (rn, subnodes) literals with
      | Some node -> node
      | None ->
	  let node = next_node in
	  next_node <- next_node+1;
