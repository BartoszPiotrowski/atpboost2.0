...
let cop_action_fl fli =
  ...
  let hashes_sorted = hashes |> List.sort_uniq (fun x y -> compare (Logic.hash_to_fl_index x) (Logic.hash_to_fl_index y)) in
...
let print_state_indices () =
  let ( [ node_input1; node_input2; node_input3; symbol_input ],
	[ node_c_input; clause_input ],
	[ ini_nodes; ini_symbols; ini_clauses ] ) = cop_graph_indices () in
  let mask_axiom_only = cop_unif_mask ()
...
let _ = Callback.register "cop_print_state_indices" print_state_indices;;
