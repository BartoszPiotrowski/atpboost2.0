...
def graph_start(graph, start_dims):
    ...
    with tf.variable_scope(None, "embeddings"):
        node_emb = tf.get_variable("node_emb", [4, dim_nodes])
        symbol_emb = tf.zeros([2, dim_symbols])
        clause_emb = tf.get_variable("clause_emb", [3, dim_nodes])
...
def segment_minimax(data, segment_indices):
    a = tf.segment_max(data, segment_indices)
    b = tf.segment_min(data, segment_indices)
    return a+b
...
def graph_conv(tensors, s, output_dims = None, use_layer_norm = False):
    ...
    for n in s.node_inputs:
        ...
        x = n.segments.collapse_nonzero(tf.nn.relu(xs + xn),
                                        [tf.segment_max, tf.segment_mean])
    ...
    x = nc.segments.collapse_nonzero(x, [tf.segment_max, tf.segment_mean])
    ...
    x = sy.segments.collapse(x*tf.expand_dims(sy.sgn,1),
                         operations = [tf.segment_mean, segment_minimax])
