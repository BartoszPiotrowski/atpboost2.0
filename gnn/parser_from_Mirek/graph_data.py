...
class GraphHyperEdgesA():
    ...
    def pretty_print(self):
        ...
        for i in range(2):
            print("  nodes[:,{}]:".format(i), self.nodes[:,i])
        print("  sgn:", self.sgn)
    ...
    def edge_perm(self, i, perm):
        l = len(perm)
        assert sorted(perm) == list(range(l))
        assert self.lens[i] == l, (l, self.lens, i)
        start_i = np.sum(self.lens[:i])
        perm_src = list(range(start_i, start_i+l))
...
class GraphHyperEdgesB():
    ...
    def edge_perm(i, perm):
        l = len(perm)
        assert sorted(perm) == list(range(l))
        assert self.lens[i] == l
        perm_src = list(start_i, start_i+l)
        perm_dest = np.array(perm, dtype = int)+start_i
        self.nodes[perm_src] = self.symbols[perm_dest]
        self.sgn[perm_src] = self.symbols[perm_dest]
...
class GraphData():
    ...
    def pretty_print(self):
        for i,hedges in enumerate(self.node_inputs):
            print("node_inputs_{}/".format(i+1))
            hedges.pretty_print()
        print("symbol_inputs/")
        self.symbol_inputs.pretty_print()
        print("node_c_inputs/")
        self.node_c_inputs.pretty_print()
        print("clause_inputs/")
        self.clause_inputs.pretty_print()
        print("ini_nodes:", self.ini_nodes)
        print("ini_symbols:", self.ini_symbols)
        print("ini_clauses:", self.ini_clauses)
